...
- **Recovered completion pass (2026-07-27)** The 2026-03-10 engagement-only challenge scoring override is final and supersedes the older Slice 05 formula wherever the documents conflict. Club and Organization event creation is now part of V1 and supersedes the 2026-04-28 admin-only event-write restriction: these roles may insert only self-attributed events, while admin event management remains available. Admin challenge management includes editing any challenge and closing an active challenge by setting `ends_at` to the current timestamp; `created_by_admin` is not rewritten during edits.
